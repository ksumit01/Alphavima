        ...
        <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
            <entity name='avpx_invoice'>
                ...
                if (orderType == 783090000) //rental
                {
                    ...
                    placeholders["pxInvoiceNumber"] = string.IsNullOrEmpty(rentalOrderId) || string.IsNullOrEmpty(invoiceNo) ? "" : rentalOrderId + " - " + invoiceNo;
                    placeholders["pxCNatJS"] = GetValueOrDefault("av_contactnameatjobsite");
                    placeholders["pxCTatJS"] = GetValueOrDefault("av_contacttelephoneatjobsite");
                    placeholders["pxJobName"] = GetValueOrDefault("av_jobname");
                    placeholders["pxDeliveryIns"] = GetValueOrDefault("av_specialdeliveryinstructions");
                    placeholders["pxAdditionalTerms"] = GetValueOrDefault("av_additionalterms");
            ...
            try
            {
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        {
                            recordData = new string[] { avpxQuantity, displayName, avpxAmount, avpxExtendedAmount };
