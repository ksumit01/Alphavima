...
namespace AV.Prexa.RentalReservation.PopulateWordTemplate
{
    ...
    <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
        <entity name='avpx_rentalReservation'>
            ...
            <attribute name='av_additionalterms' />
            <attribute name='av_contacttelephoneatjobsite' />
            <attribute name='av_contactnameatjobsite' />
            <attribute name='av_specialdeliveryinstructions' />
            ...
            var placeholders = new Dictionary<string, string>
{
    ...
    <entity name='avpx_rentalreservation' >
        ...
        <attribute name='av_contactnameatjobsite' />
        <attribute name='av_specialdeliveryinstructions' />
        <attribute name='av_cardprocessingfee' />
        ...
        <link-entity name='account' from='accountid' to='avpx_serviceaccount' link-type='inner' alias='ac' >
            ...
            <attribute name='paymenttermscode' />
                ...
                if (rentalReservation.Contains("ac.paymenttermscode"))
                {
                    ...
                    switch (paymentTermsValue)
                    {
                        ...
                        case 783090000:
                            ...
                            break;
                        case 28:
                            placeholders["pxPaymentTerms"] = "Net 28";
                            break;
                        default:
                            placeholders["pxPaymentTerms"] = "NA";
                            break;
                    }
                }

                placeholders["pxSDate"] = estimatedStartDate.ToString("MM/dd/yyyy");
                placeholders["pxEDate"] = estimatedEndtDate.ToString("MM/dd/yyyy");
                ...
                placeholders["pxRentalOut"] = rentalOutLocal.ToString("MM/dd/yyyy");
                placeholders["pxRentalIn"] = rentalInLocal.ToString("MM/dd/yyyy");
                ...
                placeholders["pxCellNo"] = GetValueOrDefault("av_contacttelephoneatjobsite");
                ...
                placeholders["pxprintedOn"] = printedOnToLocal.ToString("MM/dd/yyyy");
                placeholders["pxSDI"] = GetValueOrDefault("av_specialdeliveryinstructions");
                placeholders["pxContactName"] = GetValueOrDefault("av_contactnameatjobsite");
                placeholders["pxAddTerms"] = GetValueOrDefault("av_additionalterms");
                placeholders["pxCPFAmt"] = GetFormattedValueOrDefault("av_cardprocessingfee");
            ...
            <attribute name='avpx_damagewaiverpercentage' />
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        string dailyAmount = "N/A";
                        string weeklyAmount = "N/A";
                        string monthlyAmount = "N/A";
                        ...
                        {
                            // Retrieve the entity
                            Entity partEntity = service.Retrieve("avpx_device", assetGroupId, new ColumnSet("avpx_name", "avpx_dailyamount", "avpx_weeklyamount", "avpx_monthlyamount"));

                            // Retrieve and assign the name
                            avpxAssetGroupName = partEntity.Contains("avpx_name") ? partEntity["avpx_name"].ToString() : string.Empty;

                            // Retrieve and convert daily, weekly, and monthly amounts
                            dailyAmount = partEntity.Contains("avpx_dailyamount") ? $"${((Money)partEntity["avpx_dailyamount"]).Value:F2}" : "N/A";
                            weeklyAmount = partEntity.Contains("avpx_weeklyamount") ? $"${((Money)partEntity["avpx_weeklyamount"]).Value:F2}" : "N/A";
                            monthlyAmount = partEntity.Contains("avpx_monthlyamount") ? $"${((Money)partEntity["avpx_monthlyamount"]).Value:F2}" : "N/A";

                            // Log tracing information
                            tracingService.Trace("Part Name: {0}", avpxAssetGroupName);
                            tracingService.Trace("Daily Amount: {0}", dailyAmount);
                            tracingService.Trace("Weekly Amount: {0}", weeklyAmount);
                            tracingService.Trace("Monthly Amount: {0}", monthlyAmount);
                        }
                        else
                        {
                        ...
                        {
                            string[] recordData = new string[] { displayName, avpxQuantity,dailyAmount,weeklyAmount,monthlyAmount, avpxPricePerUnit, avpxAmount };
                        ...
                        {
                            string[] recordData = new string[] { displayName, avpxQuantity, avpxPricePerUnit, avpxAmount };
