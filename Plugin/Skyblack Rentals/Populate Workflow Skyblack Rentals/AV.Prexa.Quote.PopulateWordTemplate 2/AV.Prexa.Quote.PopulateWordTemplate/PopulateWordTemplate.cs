...
namespace AV.Prexa.Quote.PopulateWordTemplate
{
            ...
            var placeholders = new Dictionary<string, string>
    {
        ...
        {"pxJN","" },
            ...
            <attribute name='av_jobname' />
            <attribute name='av_specialdeliveryinstructions' />
            <attribute name='av_environmentfeeamount' />
             <attribute name='av_cardprocessingfeeamount' />
            ...
            <link-entity name='account' from='accountid' to='avpx_account' link-type='inner' alias='ac'>
                ...
                placeholders["pxInst"] = GetValueOrDefault("av_specialdeliveryinstructions");
                placeholders["pxCNJ"] = GetValueOrDefault("av_contactnameatjobsite");
                placeholders["pxCTJ"] = GetValueOrDefault("av_contacttelephoneatjobsite");
                placeholders["pxJN"] = GetValueOrDefault("av_jobname");
                placeholders["pxCPFAmt"] = GetFormattedValueOrDefault("av_cardprocessingfeeamount");
                placeholders["pxEnvFeeAmt"] = GetFormattedValueOrDefault("av_environmentfeeamount");
                return placeholders;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        string dailyAmount = "N/A", weeklyAmount = "N/A", monthlyAmount = "N/A";

                        if (assetGroupId != Guid.Empty)
                        {
                            Entity partEntity = service.Retrieve("avpx_device", assetGroupId, new ColumnSet("avpx_dailyamount", "avpx_weeklyamount", "avpx_monthlyamount"));
                        ...
                        if (avpxType == "Asset group")
                        {
                            recordData = new string[] { displayName, avpxQuantity, dailyAmount, weeklyAmount, monthlyAmount, avpxAmount, avpxExtendedAmount };
                            assetGroup.Add(recordData);
                        }
                        else if (avpxType == "Charge")
                        {
                            recordData = new string[] { displayName, avpxQuantity, avpxAmount, avpxExtendedAmount };
