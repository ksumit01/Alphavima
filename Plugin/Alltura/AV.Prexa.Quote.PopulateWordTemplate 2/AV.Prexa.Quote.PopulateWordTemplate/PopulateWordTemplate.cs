...
namespace AV.Prexa.Quote.PopulateWordTemplate
{
    public class PopulateWordTemplate : CodeActivity
    {
        ...
        static string pxPlataforma;
        static string pxCapacidad;
        static string pxAlto;
        static string pxTrabajo;
        static string pxPeso;
        static string pxAlcance;
        static string pxAncho;
        static string pxFuerza;
        static string pxLargo;
            ...
            var placeholders = new Dictionary<string, string>
    {
        ...
        {"pxNotes","" },
            ...
            <link-entity name='account' from='accountid' to='avpx_account' link-type='inner' alias='ac'>
                ...
                placeholders["pxNotes"] = GetValueOrDefault("av_notes");
        ...
        static byte[] WordTemplate(byte[] fileData, byte[] imageData, Dictionary<string, string> placeholders, List<string[]> combinedData, ITracingService tracingService)
        {
            ...
            try
            {
                using (MemoryStream memoryStream = new MemoryStream())
                {
                 ...
                 ;
                        tracingService.Trace("pxPlataforma" + pxPlataforma);
                        tracingService.Trace("pxAlto" + pxAlto);
                        tracingService.Trace("pxCapacidad" + pxCapacidad);
                        tracingService.Trace("pxAlcance" + pxAlcance);
                        tracingService.Trace("pxPeso" + pxPeso);
                        tracingService.Trace("pxTrabajo" + pxTrabajo);
                        tracingService.Trace("pxAncho"+ pxAncho);
                        tracingService.Trace("pxFuerza" + pxFuerza);
                        tracingService.Trace("pxLargo" + pxLargo);

                        placeholders["pxPlataforma"] = pxPlataforma;
                        placeholders["pxAlto"] = pxAlto;
                        placeholders["pxCapacidad"] = pxCapacidad;
                        placeholders["pxTrabajo"] = pxTrabajo;
                        placeholders["pxPeso"] = pxPeso;
                        placeholders["pxAlcance"] = pxAlcance;
                        placeholders["pxAncho"] = pxAncho;
                        placeholders["pxFuerza"] = pxFuerza;
                        placeholders["pxLargo"] = pxLargo;
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        if (assetId != Guid.Empty)
                        {
                            ...
                            Entity assetEntity = service.Retrieve("avpx_asset", assetId, new ColumnSet("avpx_name", "av_alturadeplataformam","av_altodelequipom", "av_largodelequipom", "av_alcancehorizontalm", "av_capacidaddecargakg", "av_alturadetrabajom", "av_pesodelequipokg","av_anchodelequipom", "av_fuerzamotriz"));
                            assetName = GetValueAsString(assetEntity, "avpx_name", service,tracingService);
                            pxPlataforma = GetFormattedValueOrDefault(assetEntity, "av_alturadeplataformam", tracingService);
                            pxCapacidad = GetFormattedValueOrDefault(assetEntity, "av_capacidaddecargakg",tracingService);
                            pxAlto = GetFormattedValueOrDefault(assetEntity, "av_altodelequipom", tracingService);
                            pxTrabajo = GetFormattedValueOrDefault(assetEntity, "av_alturadetrabajom", tracingService);
                            pxPeso = GetFormattedValueOrDefault(assetEntity, "av_pesodelequipokg",tracingService) ;
                            pxLargo = GetFormattedValueOrDefault(assetEntity, "av_largodelequipom", tracingService);
                            pxAlcance = GetFormattedValueOrDefault(assetEntity, "av_alcancehorizontalm", tracingService);
                            pxAncho = GetFormattedValueOrDefault(assetEntity, "av_anchodelequipom", tracingService);
                            pxFuerza = GetValueAsString(assetEntity, "av_fuerzamotriz",service, tracingService);
                        }
                        else
                        {
                        ...
                        string[] recordData = new string[] { avpxName, avpxQuantity, avpxAmount, avpxExtendedAmount };
