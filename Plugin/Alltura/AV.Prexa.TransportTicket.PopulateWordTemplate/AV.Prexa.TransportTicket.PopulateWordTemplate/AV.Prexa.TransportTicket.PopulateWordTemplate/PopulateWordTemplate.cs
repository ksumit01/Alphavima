            ...
            <condition attribute='avpx_ticketid' operator='eq' value='{0}' />
                ...
                /*string GetFormattedValueOrDefault(string attribute, string defaultValue = "")
                {
                    try
                    {
                        if (transportTicket.FormattedValues.Contains(attribute))
                        {
                            var value = initiaTransportTicket.FormattedValues[attribute];
                            if (decimal.TryParse(value, out decimal decimalValue))
                            {
                                return decimalValue.ToString("N2"); // Format decimal values with two decimal places
                            }
                ...
                string GetAliasedValueOrDefaultName(string attribute, string defaultValue = "")
                {
                    try
                    {
                        return transportTicket.Contains(attribute) ? (transportTicket[attribute] as AliasedValue)?.Value.ToString() ?? defaultValue : defaultValue;
                    }
                    catch (Exception ex)
                    {
                        tracingService.Trace("Error retrieving aliased value for attribute " + attribute + ": " + ex.ToString());
                ...
                if (transportType == 783090001) //Pickup
                {
                    ...
                    placeholders["pxtransportDate"] = pickUpDateOnUtcLocal.ToString("dd/MM/yyyy");
                }
                else if (transportType == 783090000) //Dispatch
                {
                    ...
                    placeholders["pxtransportDate"] = dispatchDateOnUtcLocal.ToString("dd/MM/yyyy");
            ...
            <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        string[] recordData = new string[] { avpxSerialNumber, assetName, avpxMake, avpxModel };
