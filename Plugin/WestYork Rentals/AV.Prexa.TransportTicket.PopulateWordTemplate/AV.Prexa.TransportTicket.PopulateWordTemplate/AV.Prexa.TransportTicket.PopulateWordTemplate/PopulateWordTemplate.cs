...
namespace AV.Prexa.TransportTicket.PopulateWordTemplate
{
    ...
    <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
        ...
        public string GetTransportTicketFetchXml(Guid transportTicketId, int ticketType, int? transportType)
        {
            string dynamicLinkEntity = "";
            string transportAttributes = "";

            // Adding dynamic link-entity based on ticketType
            switch (ticketType)
            {
    ...
    <attribute name='avpx_driverdetails' />";
            ...
            // Adding logic for transportType
            if (transportType.HasValue && transportType.Value == 783090002)
            {
                transportAttributes = @"
    <attribute name='av_onsitecontact' />";
    
            }

            // Combining everything into the fetchXml
...
<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
    <entity name='avpx_ticket'>
        ...
        {1}  <!-- ticketType-based dynamic link -->
        {2}  <!-- transportType-based attributes and link entity -->
    </entity>
</fetch>", transportTicketId, dynamicLinkEntity, transportAttributes);
            ...
            var placeholders = new Dictionary<string, string>
    {
        ...
        {"pxTicket","" }
            ...
            try
            {
                ...
                // Instead of passing just ticketType, pass both ticketType and transportType
                string dynamicFetchXml = GetTransportTicketFetchXml(new Guid(transportTicketId),
                                                                    ticketType.HasValue ? ticketType.Value : 0,
                                                                    transportType);
                ...
                DateTime transportDateLocal = GetLocalTime(transportDateUtc, userTimeZone, service, tracingService);
                ...
                tracingService.Trace($"Transport Date (Local): {transportDateLocal}");
                ...
                placeholders["pxTicket"] = GetValueOrDefault("avpx_ticketno");
                ...
                if (transportType == 783090001) //Pickup
                {
                    ...
                    //placeholders["pxNotes"] = GetValueAsString(transportTicket, "avpx_purpose", service, tracingService);
                    ...
                    placeholders["pxNotes"] = GetValueOrDefault("av_onsitecontact");
...
*/
                    ...
                    placeholders["pxPickInstruction"] = GetValueOrDefault( "avpx_purpose");
                    //placeholders["transportTicketNumber"] = transportTicketNo;
                    placeholders["pxNotes"] = GetValueOrDefault("av_onsitecontact");
                }
                else if(transportType == 783090002)
                {
                    placeholders["pxStreetAddress"] = GetValueOrDefault("avpx_jsl_streetaddress");
                    placeholders["pxCity"] = GetValueOrDefault("avpx_jsl_city");
                    placeholders["pxState"] = GetValueOrDefault("avpx_jsl_stateorprovince");
                    ...
                    placeholders["pxcusP"] = GetValueOrDefault("avpx_brl_postalcode");
                    placeholders["pxcusCo"] = GetValueOrDefault("avpx_brl_country");
                    placeholders["pxtransportDate"] = transportDateLocal.ToString("MM/dd/yyyy");
                    placeholders["pxNotes"] = GetValueOrDefault("av_onsitecontact");
                    placeholders["pxPickInstruction"] = GetValueOrDefault("avpx_purpose");

                }
                placeholders["pxPO"] = GetValueOrDefault("av_po");
        ...
        <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
            <entity name='avpx_ticketitem'>
                <attribute name='avpx_transportticket' />
                <attribute name='avpx_asset' />
                <attribute name='av_assettype' />
                <attribute name='av_unitnumber' />
                <attribute name='av_serialnumber' />
                <attribute name='av_make' />
                <attribute name='av_model' />
                <attribute name='av_hoursout' />
                <attribute name='av_fuelsource' />
                <attribute name='av_unitinfo' />
                <filter type='and'>
                    <condition attribute='statuscode' operator='eq' value='1' />
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    {
                        Guid transportTicketId = entity.GetAttributeValue<EntityReference>("avpx_transportticket")?.Id ?? Guid.Empty;

                        // Retrieve the transport ticket to get the transport type
                        Entity transportTicketEntity = service.Retrieve("avpx_ticket", transportTicketId, new ColumnSet("avpx_transporttype"));
                        int? transportType = transportTicketEntity.GetAttributeValue<OptionSetValue>("avpx_transporttype")?.Value;

                        if (transportType == 783090000 || transportType == 783090001)
                        {
                            tracingService.Trace("Transport Type is 783090000 or 783090001. Running current logic.");

                            string avpxMake = "N/A";
                            string avpxModel = "N/A";
                            string avpxSerialNumber = "N/A";
                            string assetName = "N/A";
                            string meterReading1 = "N/A";
                            string fuelSource = "N/A";
                            ...
                            if (assetId != Guid.Empty)
                            {
                                ...
                                meterReading1 = GetValueOrDefault(assetEntity, "avpx_meter1reading", "N/A");
                                fuelSource = GetValueOrDefault(assetEntity, "av_fuelsource", "N/A");

                                tracingService.Trace("Serial Number: " + avpxSerialNumber);
                                tracingService.Trace("Asset Name: " + assetName);
                                tracingService.Trace("Meter Reading: " + meterReading1);
                                tracingService.Trace("Fuel Source: " + fuelSource);
                            }

                            string[] serializedData = new string[] { avpxSerialNumber, assetName, avpxMake, avpxModel, meterReading1, fuelSource };
                            serializedItems.Add(serializedData);
                        }
                        else if (transportType == 783090002)
                        {
                            string avpxMake = "N/A";
                            ...
                            string meterReading1 = "N/A";
                            string fuelSource = "N/A";
                            tracingService.Trace("Transport Type is 783090002. Running alternative logic.");

                            // Fetch the transport ticket to retrieve billTo value
                           /* Entity transportTicketEntityWithBillTo = service.Retrieve("avpx_ticket", transportTicketId, new ColumnSet("av_billto"));
                            int billTo = transportTicketEntityWithBillTo.Contains("av_billto") ? transportTicketEntityWithBillTo.GetAttributeValue<OptionSetValue>("av_billto").Value: 0;

                            tracingService.Trace("billTo" + billTo);*/

                            string assetType = GetValueAsString(entity, "av_assettype",service,tracingService); 
                            if (assetType == "Inhouse")
                            {
                                Guid assetId = entity.GetAttributeValue<EntityReference>("avpx_asset")?.Id ?? Guid.Empty;
                                tracingService.Trace("assetId");

                                if (assetId != Guid.Empty)
                                {
                                    // Retrieve the asset lookup to get the name
                                ...
                                {
                                    tracingService.Trace("Asset Id Not Found");
                                }

                                
                            }
                            else if (assetType == "External")
                            {
                                tracingService.Trace("Bill To is 121010001: Retrieving Unit Info only.");

                                 assetName = GetValueOrDefault(entity, "av_unitinfo", "N/A"); ;
                               
                                
                            }

                            avpxMake = GetValueOrDefault(entity,"av_make","");
                            avpxSerialNumber = GetValueOrDefault(entity, "av_serialnumber", "");
                            avpxModel = GetValueOrDefault(entity, "av_model", "");
                            meterReading1 = GetValueOrDefault(entity, "av_hoursout", "");
                            fuelSource = GetValueOrDefault(entity, "av_fuelsource", "");
                            tracingService.Trace("MAke" + avpxMake);
                            tracingService.Trace("Asset Name retrieved: " + assetName);
                            tracingService.Trace("av_serialnumber" + avpxSerialNumber);
                            tracingService.Trace("avpxModel" + avpxModel);
                            tracingService.Trace("meterReading1" + meterReading1);
                            tracingService.Trace("fuelSource" + fuelSource);
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        if (assetGroupId != Guid.Empty)
                        {
                            ...
                            avpxAssetGroupName = assetGroupEntity.Contains("avpx_name") ? assetGroupEntity["avpx_name"].ToString() : "N/A";
                        }

                        avpxQuantity = GetFormattedValueOrDefault(entity, "avpx_quantity", tracingService, "N/A");
                        string[] recordData = new string[] { avpxAssetGroupName, avpxQuantity };
        ...
        private string GetValueOrDefault(Entity entity, string attributeName, string defaultValue)
        {
            if (entity.Contains(attributeName))
            {
                ...
                // Check if the value is numeric (decimal or double) and format it to two decimal places.
