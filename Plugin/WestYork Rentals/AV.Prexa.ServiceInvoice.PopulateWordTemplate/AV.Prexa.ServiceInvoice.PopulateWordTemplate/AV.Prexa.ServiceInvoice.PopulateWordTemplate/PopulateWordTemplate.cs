            ...
            var placeholders = new Dictionary<string, string>
    {
        ...
        {"pxTrans","" }
            ...
            try
            {
                        ...
                        <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                          <entity name='avpx_serviceinvoice'>
                            ...
                            <filter type='and' >
                                ...
                                <condition attribute='avpx_serviceinvoiceid' operator='eq' value='{0}' />
                            </filter>
                          </entity>
                        </fetch>", invoiceId); 
                tracingService.Trace("Initial Fetch Expression: " + initialFetch);

                EntityCollection serviceInvoiceCollection;
                try
                {
                    serviceInvoiceCollection = service.RetrieveMultiple(new FetchExpression(initialFetch));
                }
                catch (Exception ex)
                {
                    tracingService.Trace("Error retrieving service order: " + ex.ToString());
                    throw new InvalidPluginExecutionException("Failed to retrieve service order. " + ex.Message);
                }
                if (serviceInvoiceCollection == null || serviceInvoiceCollection.Entities.Count == 0)
                {
                    tracingService.Trace("No invoices found with the given ID.");
                    return placeholders;
                }

                Entity invoiceInitial = serviceInvoiceCollection.Entities[0];
                Guid serviceOrderId = invoiceInitial.GetAttributeValue<EntityReference>("avpx_serviceorder")?.Id ?? Guid.Empty;

                if (serviceOrderId == Guid.Empty)
                {
                    tracingService.Trace("Invalid service order ID.");
                    return placeholders;
                }

                // Step 2: Retrieve the Full Service Order with Relevant Attributes
                Entity serviceOrderEntity;
                try
                {
                    serviceOrderEntity = service.Retrieve("avpx_serviceorder", serviceOrderId, new ColumnSet("avpx_serviceordertype", "avpx_billto", "avpx_customer"));
                }
                catch (Exception ex)
                {
                    tracingService.Trace("Error retrieving service order: " + ex.ToString());
                ...
                OptionSetValue serviceOrderTypes = serviceOrderEntity.GetAttributeValue<OptionSetValue>("avpx_serviceordertype");
                OptionSetValue billTo = serviceOrderEntity.GetAttributeValue<OptionSetValue>("avpx_billto");
                EntityReference customer = serviceOrderEntity.GetAttributeValue<EntityReference>("avpx_customer");


                bool isInHouse = serviceOrderTypes?.Value == 783090000;  // In-house
                bool isBillToInternal = billTo?.Value == 783090000;      // Internal
                bool hasCustomer = customer != null;

                tracingService.Trace($"Service Order Type: {serviceOrderTypes?.Value}, Bill To: {billTo?.Value}, Customer Present: {hasCustomer}");

                string accountLinkEntity = "";

                if (!isInHouse || !isBillToInternal || hasCustomer)
                {
                    // Add the account link entity only if customer data exists or in other scenarios
                    accountLinkEntity = @"
                ...
                tracingService.Trace("After Account Entity");
                ...
                string invoiceFetch = $@"
                    <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                      <entity name='avpx_serviceinvoice'>
                        ...
                        <attribute name='av_transportcharges' />
                        <order attribute='createdon' descending='true' />
                        <filter type='and'>
                          <condition attribute='statecode' operator='eq' value='0' />
                          <condition attribute='avpx_serviceinvoiceid' operator='eq' value='{invoiceId}' />
                        </filter>
                        <link-entity name='avpx_serviceorder' from='avpx_serviceorderid' to='avpx_serviceorder' link-type='outer' alias='aa'>
                          ...
                          <attribute name='avpx_startdate' />
                          <attribute name='avpx_notes' />
                          <attribute name='av_ponumber' />
                          <attribute name='av_consumables' />
                          <attribute name='av_siteaddress' />
                          <attribute name='avpx_ordernumber' />
                        </link-entity>
                        {accountLinkEntity} <!-- Dynamically added based on conditions -->
                      </entity>
                    </fetch>";
            ...
            string TotalLabourHours = "0.00"; // Default value if no valid result is found
                string unitInfo = "";
                Guid serviceOrderID = invoice.GetAttributeValue<EntityReference>("avpx_serviceorder")?.Id ?? Guid.Empty;

                
                if (serviceOrderID != Guid.Empty)
                {
                    // Retrieve the service order entity to get avpx_serviceunit and avpx_serviceordertype
                    Entity serviceOrder = service.Retrieve("avpx_serviceorder", serviceOrderID, new ColumnSet("avpx_serviceunit", "avpx_serviceordertype","avpx_serviceunitdetails"));

                    if (serviceOrder.Contains("avpx_serviceordertype"))
                    {
                        int serviceOrderType = serviceOrder.GetAttributeValue<OptionSetValue>("avpx_serviceordertype").Value;

                        if (serviceOrderType == 783090000) // In-house
                        {
                            // Continue existing behavior for In-house
                            if (serviceOrder.Contains("avpx_serviceunit") && serviceOrder["avpx_serviceunit"] is EntityReference serviceUnitReference)
                            {
                                Guid serviceUnitID = serviceUnitReference.Id;

                                // Step 2: Retrieve the av_unitInfo field from the avpx_serviceunit entity
                                Entity serviceUnit = service.Retrieve("avpx_asset", serviceUnitID, new ColumnSet("av_unitinfo"));

                                if (serviceUnit.Contains("av_unitinfo"))
                                {
                                    // Get the av_unitInfo value
                                    unitInfo = serviceUnit.GetAttributeValue<string>("av_unitinfo");

                                    // Use the unitInfo as needed
                                    tracingService.Trace("Unit Info: " + unitInfo);
                                }
                                else
                                {
                                    tracingService.Trace("av_unitInfo not found.");
                                }
                            }
                            else
                            {
                                tracingService.Trace("avpx_serviceunit lookup not found on service order.");
                            }
                        }
                        else if (serviceOrderType == 783090001) // Walk-In
                        {
                            // Fetch avpx_serviceunitdetails instead for Walk-In
                            if (serviceOrder.Contains("avpx_serviceunitdetails"))
                            {
                                // Retrieve the avpx_serviceunitdetails field value
                                unitInfo = serviceOrder.GetAttributeValue<string>("avpx_serviceunitdetails");

                                // Use the unitInfo as needed
                                tracingService.Trace("Unit Info (Walk-In): " + unitInfo);
                            }
                            else
                            {
                                tracingService.Trace("avpx_serviceunitdetails not found on service order.");
                            }
                        }
                        else
                        {
                            tracingService.Trace("Unknown service order type.");
                        }
                    }
                    else
                    {
                        tracingService.Trace("avpx_serviceordertype not found on service order.");
                    }
                }
                else
                {
                    tracingService.Trace("Invalid serviceOrderID.");
                }
                ...
                if (serviceOrderID != Guid.Empty)
                {
                    try
                    {
                        // FetchXML query with aggregation to sum 'avpx_labourhours'
                        string totalHourFetch = string.Format(@"
            <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false' aggregate='true'>
                <entity name='avpx_servicejob'>
                    <attribute name='avpx_labourhours' alias='totalhours' aggregate='sum' />
                    <filter type='and'>
                        <condition attribute='avpx_serviceorder' operator='eq' value='{0}' />
                        <condition attribute='statecode' operator='eq' value='0' />
                    </filter>
                </entity>
            </fetch>", serviceOrderID);

                        // Execute the FetchXML query
                        EntityCollection result = service.RetrieveMultiple(new FetchExpression(totalHourFetch));

                        // Check if result is not null and contains at least one entity
                        if (result != null && result.Entities.Count > 0)
                        {
                            // Retrieve the aggregated sum using the alias 'totalhours'
                            var totalHoursAliasedValue = result.Entities[0].GetAttributeValue<AliasedValue>("totalhours");

                            // Ensure the alias exists and has a value
                            if (totalHoursAliasedValue != null && totalHoursAliasedValue.Value != null)
                            {
                                // Cast to decimal (or use appropriate type depending on your system)
                                decimal? totalHours = totalHoursAliasedValue.Value as decimal?;

                                // If totalHours has a value, assign it to TotalLabourHours
                                if (totalHours.HasValue)
                                {
                                    TotalLabourHours = totalHours.Value.ToString("F2");
                                }
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        ...
                        tracingService.Trace("Error retrieving total labor hours: " + ex.Message);
                    }
                }
                else
                {
                    // Handle the case where serviceOrderID is invalid
                    tracingService.Trace("Invalid Service Order ID.");
                }
                ...
                string GetValueOrDefault(string attribute, string defaultValue = "")
                {
                    try
                    {
                        if (invoice.Contains(attribute))
                        {
                            ...
                            if (value is Money moneyValue)
                            {
                                // Format money value as a string and apply regex to remove any country prefix before the dollar sign
                                var moneyString = moneyValue.Value.ToString("C2"); // Convert to currency format with symbol
                            ...
                            if (value is string htmlContent)
                            {
                                ...
                                return ExtractTextFromHtml(htmlContent) ?? defaultValue;
                            }
                ...
                string ExtractTextFromHtml(string htmlContent)
                {
                    ...
                    // Extract the inner text from the document node
                    var text = doc.DocumentNode.InnerText;

                    // Decode HTML entities (e.g., &nbsp; to a regular space)
                ...
                DateTime serviceDateUtc = DateTime.Parse(GetAliasedValueOrDefault("aa.avpx_startdate", DateTime.UtcNow.ToString("o")));
                ...
                DateTime serviceDatUtcLocal = GetLocalTime(serviceDateUtc, userTimeZone, service, tracingService);
                ...
                placeholders["pxLabourchge"] = GetFormattedValueOrDefault("avpx_totallabourcharges");
                ...
                placeholders["pxCustomerNo"] = GetAliasedValueOrDefault("aa.avpx_ordernumber");
                placeholders["pxCustomerName"] = fullName;
                placeholders["pxServiceDate"] = serviceDatUtcLocal.ToString("MM/dd/yyyy");
                placeholders["pxEmail"] = GetAliasedValueOrDefault("ac.emailaddress1");
                placeholders["pxNotes"] = GetValueOrDefault("av_description");
                //placeholders["pxEmail"] = emailContact;
                placeholders["pxPartsTotal"] = GetFormattedValueOrDefault("avpx_totalpartskitscharges");
                placeholders["pxLabourChrg"] = TotalLabourHours;
                placeholders["pxPoNumber"] = GetAliasedValueOrDefault("aa.av_ponumber");
                placeholders["pxSiteAdd"] = GetAliasedValueOrDefault("aa.av_siteaddress");
                placeholders["pxUnitInfo"] = unitInfo;
                placeholders["pxCnsbl"] = GetAliasedValueOrDefault("aa.av_consumables");
                placeholders["pxAccName"] = GetAliasedValueOrDefault("ac.name");
                placeholders["pxTrans"] = GetFormattedValueOrDefault("av_transportcharges");
            ...
            try
            {
                using (MemoryStream memoryStream = new MemoryStream())
                {
                 ...
                 ;
                        ...
                        if (charges.Count == 0)
                        {
                            tracingService.Trace("Charges section is empty. Attempting to modify tables.");

                           // var tables = wordDoc.MainDocumentPart?.Document?.Body?.Descendants<DocumentFormat.OpenXml.Wordprocessing.Table>().ToList();
                            if (tables == null || !tables.Any())
                            {
                                tracingService.Trace("No tables found in the document.");
                                return null; // Exit if there are no tables to modify
                            }
                            ...
                            foreach (var table in tables)
                            {
                                ...
                                if (tableCaption != null)
                                {
                                    ...
                                    if (altText == null)
                                    {
                                        tracingService.Trace("Table caption alt text is null. Skipping this table.");
                                        continue; // Skip if alt text is null
                                    }

                                    // Check for the 'Charges' table to delete it
                                    if (altText.Contains("Charges"))
                                    {
                                        tracingService.Trace("Deleting the table with alt text 'Charges'.");
                                        table.Remove();
                                        tracingService.Trace("Table with alt text 'Charges' removed.");
                                        continue; // Continue to the next iteration after deleting the table
                                    }

                                    // Check for the 'AmountTable' to remove the second row
                                    if (altText.Contains("AmountTable"))
                                    {
                                        tracingService.Trace("Found table with alt text 'AmountTable'.");
                                        var rows = table.Elements<DocumentFormat.OpenXml.Wordprocessing.TableRow>().ToList();

                                        if (rows.Count > 1)
                                        {
                                            tracingService.Trace("Removing the second row of the 'AmountTable'.");
                                            rows[1].Remove(); // Removes the second row (index 1)
                                        }
                                        else
                                        {
                                            tracingService.Trace("The 'AmountTable' does not have a second row to remove.");
                                        }
                                    }
                                }
                                else
                                {
                                    tracingService.Trace("Table does not have a caption. Skipping this table.");
                                }
                            }
                        }
            ...
            try
            {
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        if (avpxType == "Charge")
                        {
                            recordData = new string[] { avpxName, avpxAmount, avpxExtendedAmount };
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        string avpxExtendedAmount = GetFormattedValueOrDefault(entity, "avpx_amount", tracingService);
                        string avpxNewExtendedAmount = GetFormattedValueOrDefault(entity, "avpx_extendedamount",tracingService);
                        decimal localTotalPartKitAmount = GetRawValue(entity, "avpx_sellingprice");
                        tracingService.Trace("localTotalPartKitAmount" + localTotalPartKitAmount);
                        decimal totalQuantity = GetRawValue(entity, "avpx_quantity");
                        tracingService.Trace("totalQuantity" + totalQuantity);

                        // decimal localTotalPartKit = localTotalPartKitAmount * totalQuantity;

                        //tracingService.Trace("localTotalPartKit" + localTotalPartKit);

                        if (localTotalPartKitAmount != 0 && totalQuantity != 0)
                        {
                            decimal localTotalPartKit = localTotalPartKitAmount * totalQuantity;

                            // Call the custom rounding logic
                            localTotalPartKit = RoundUpBasedOnLastDigit(localTotalPartKit,tracingService);

                            tracingService.Trace("localTotalPartKit (rounded): " + localTotalPartKit);
                        ...
                        {
                            tracingService.Trace("One of the values is zero, skipping calculation.");
                        }
                        tracingService.Trace("Extended Amount" + avpxExtendedAmount);
                        string avpxPartName = "";
                        string partKitId = "";

                        //TotalPartKit = FormatWithCurrency(localTotalPartKit, currencySymbol);
                       
                        Guid partId = entity.GetAttributeValue<EntityReference>("avpx_partkit")?.Id ?? Guid.Empty;
                        if (partId != Guid.Empty)
                        {
                            Entity partEntity = service.Retrieve("avpx_parts", partId, new ColumnSet("avpx_name", "avpx_partkitid"));
                            avpxPartName = partEntity.Contains("avpx_name") ? partEntity["avpx_name"].ToString() : string.Empty;
                            partKitId = partEntity.Contains("avpx_partkitid") ? partEntity["avpx_partkitid"].ToString() : string.Empty;
                        ...
                        //string[] recordData = new string[] { avpxQuantity, partKitId, avpxName, avpxAmount, TotalPartKit };
                        string[] recordData = new string[] { avpxQuantity, partKitId, displayName, avpxAmount, avpxExtendedAmount };
        ...
        private decimal RoundUpBasedOnLastDigit(decimal value, ITracingService tracingService)
        {
            tracingService.Trace("Current Value: " + value);

            // Check if the value is already at a boundary where rounding wouldn't happen (e.g., 56.82)
            decimal roundedValue = Math.Round(value, 2, MidpointRounding.AwayFromZero);

            // If the rounded value is equal to the original value, but we need to round up, manually add 0.01
            if (roundedValue == value)
            {
                roundedValue += 0.01M;
            }

            tracingService.Trace("Rounded Value: " + roundedValue);
        ...
        private decimal GetRawValue(Entity entity, string attributeName)
        {
            if (entity.Contains(attributeName))
            {
                ...
                if (attributeValue is Money moneyValue)
                {
                    // Handle Money type field
                    return moneyValue.Value;
                }
                else if (attributeValue is decimal decimalValue)
                {
                    // Handle decimal type field
                    return decimalValue;
                }
            }

            // Return 0 if the field is not found or null
            return 0;
        }
        ...
        private string RoundOffValue(Entity entity, string attributeName)
        {
            if (entity.Contains(attributeName))
            {
                ...
                if (value != null)
                {
                    ...
                    {
                        return decimalValue.ToString("F0");
                    }
                    ...
                    {
                        return parsedDecimalValue.ToString("F0");
                    }
                }
            }
            return "0.00";
