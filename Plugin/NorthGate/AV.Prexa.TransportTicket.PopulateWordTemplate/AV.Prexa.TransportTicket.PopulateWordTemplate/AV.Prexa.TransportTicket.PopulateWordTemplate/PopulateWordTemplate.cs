...
namespace AV.Prexa.TransportTicket.PopulateWordTemplate
{
    public class PopulateWordTemplate : CodeActivity
    {
        ...
        [Output("Base64Word")]
        public OutArgument<string> Base64Word { get; set; }
...
<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
    <entity name='avpx_ticket'>
        ...
        <attribute name='av_addressdetails' />
        <attribute name = 'avpx_purpose' />
        <attribute name = 'av_primarysitecontact' />
         <attribute name = 'av_addressdetailsb' />
        <order attribute='createdon' descending='true' />
        <filter type='and'>
            <condition attribute='statuscode' operator='eq' value='1' />
            <condition attribute='avpx_ticketid' operator='eq' value='{0}' />
        </filter>
<link-entity name='avpx_rentalreservation' from='avpx_rentalreservationid' to='avpx_rentalorder' link-type='outer' alias='ai' >
    ...
    <entity name='avpx_ticket' >
        ...
        <filter type='and' >
            ...
            <condition attribute='avpx_ticketid' operator='eq' value='{0}' />
                ...
                string emailContact = "";
                string contactTel = "";
                if (transportTicket.Contains("av_primarysitecontact"))
                {
                    var onSiteReference = (EntityReference)transportTicket["av_primarysitecontact"];
                    Guid onSiteRefId = onSiteReference.Id;

                    // Step 3: Retrieve the opportunitu Entity Using the GUID
                    var onSiteCon = service.Retrieve("contact", onSiteRefId,
                        new ColumnSet("firstname", "lastname", "emailaddress1", "telephone1"));

                    // Step 4: Combine the Name and Display the Details
                    if (onSiteCon != null)
                    {
                        onSiteContact = $"{onSiteCon.GetAttributeValue<string>("firstname")} " +
                                         $"{onSiteCon.GetAttributeValue<string>("lastname")}";
                        emailContact = onSiteCon.GetAttributeValue<string>("emailaddress1");
                        contactTel = onSiteCon.GetAttributeValue<string>("telephone1");
                        tracingService.Trace($"onSiteContact: {onSiteContact}");

                    }
                }
                else
                {
                    tracingService.Trace("No opportunity associated with this quote.");
                }
                ...
                try
                {
                    // Step 1: Check if the linked entity field is present
                    if (transportTicket.Contains("ai.avpx_serviceaccount"))
                    {
                        tracingService.Trace("avpx_serviceaccount field found. Attempting to extract AliasedValue...");

                        // Step 2: Retrieve the AliasedValue
                        var aliasedValue = (AliasedValue)transportTicket["ai.avpx_serviceaccount"];
                        if (aliasedValue != null && aliasedValue.Value is EntityReference accountReference)
                        {
                            Guid accountId = accountReference.Id;
                            tracingService.Trace($"avpx_serviceaccount GUID: {accountId}");

                            // Step 3: Retrieve the Account entity
                            Entity accountEntity = service.Retrieve("account", accountId, new ColumnSet("name"));
                            if (accountEntity != null)
                            {
                                accountName = accountEntity.GetAttributeValue<string>("name");
                                tracingService.Trace($"Account Name Retrieved: {accountName}");
                            }
                            else
                            {
                                tracingService.Trace("Account entity not retrieved (null).");
                            }
                        }
                        else
                        {
                            tracingService.Trace("The value of 'ai.avpx_serviceaccount' is not a valid EntityReference.");
                        }
                    }
                    else
                    {
                        tracingService.Trace("The field 'ai.avpx_serviceaccount' is not present in the transportTicket entity.");
                    }
                }
                catch (Exception ex)
                {
                ...
                DateTime expectedDateOnUTC;
                DateTime expectedDateOnUtcLocal;
                bool hasValidDate = false;
                string deliveryDateStr = GetValueOrDefault("avpx_deliverydatefrom", "");

                // Check if the deliveryDateStr is valid before parsing to DateTime
                if (!string.IsNullOrEmpty(deliveryDateStr) && DateTime.TryParse(deliveryDateStr, out expectedDateOnUTC))
                {
                    // If valid, convert the UTC time to local time
                    expectedDateOnUtcLocal = GetLocalTime(expectedDateOnUTC, userTimeZone, service, tracingService);
                    hasValidDate = true;
                }
                else
                {
                    // Handle the scenario where the string is null, empty, or not a valid DateTime
                    tracingService.Trace("The delivery date is null, empty, or invalid. Skipping local time conversion.");
                    expectedDateOnUtcLocal = DateTime.MinValue; // Assign a default value to avoid unassigned variable error
                }
                ...
                //DateTime expectedDateOnUTC = DateTime.Parse(GetValueOrDefault("avpx_deliverydatefrom",""));

                String expectedDate = GetValueOrDefault("avpx_deliverydatefrom");
                tracingService.Trace($"Printed On Date (UTC): {printedOnUtc}");
                //tracingService.Trace($"expectedDateOnUTC On Date (UTC): {expectedDateOnUTC}");
                tracingService.Trace("Expected Date" + expectedDate);
                ...
                //DateTime expectedDateOnUtcLocal = GetLocalTime(expectedDateOnUTC, userTimeZone, service, tracingService);
                ...
                //tracingService.Trace($"expectedDateOnUtcLocal: {expectedDateOnUtcLocal}");
                ...
                string type = "";
                if (transportType == 783090000)
                {
                    type = "Dispatch";
                    placeholders["pxAdd"] = GetValueOrDefault("av_addressdetailsb");
                    placeholders["pxDet"] = GetValueOrDefault("av_addressdetails");
                }
                else
                {
                    type = "Pickup";
                    placeholders["pxAdd"] = GetValueOrDefault("av_addressdetails");
                    placeholders["pxDet"] = GetValueOrDefault("av_addressdetailsb");
                }
                placeholders["pxType"] = type;
                if (transportType == 783090001) //Pickup
                {
                    ...
                    placeholders["pxcusAdd"] = GetValueOrDefault("avpx_jsl_streetaddress");
                    placeholders["pxcustomerCit"] = GetValueOrDefault("avpx_jsl_city");
                    placeholders["pxcusS"] = GetValueOrDefault("avpx_jsl_stateorprovince");
                    placeholders["pxcusP"] = GetValueOrDefault("avpx_jsl_postalcode");
                    placeholders["pxcusCo"] = GetValueOrDefault("avpx_jsl_ccountry");

                    placeholders["pxStreetAddress"] = GetValueOrDefault("avpx_brl_streetaddress");
                    placeholders["pxCity"] = GetValueOrDefault("avpx_brl_city");
                    placeholders["pxState"] = GetValueOrDefault("avpx_brl_stateorprovince");
                    placeholders["pxPostalCode"] = GetValueOrDefault("avpx_brl_postalcode");
                    placeholders["pxCountry"] = GetValueOrDefault("avpx_brl_country");
                    placeholders["pxtransportDate"] = pickUpDateOnUtcLocal.ToString("MM/dd/yyyy HH:mm:ss");
                }
                else if (transportType == 783090000) //Dispatch
                {
                    ...
                    placeholders["pxState"] = GetValueOrDefault("avpx_jsl_stateorprovince");
                    placeholders["pxPostalCode"] = GetValueOrDefault("avpx_jsl_postalcode");
                    placeholders["pxCountry"] = GetValueOrDefault("avpx_jsl_ccountry");
                    placeholders["pxtransportDate"] = dispatchDateOnUtcLocal.ToString("MM/dd/yyyy HH:mm:ss");
                    // Pass the appropriate value based on whether expectedDateOnUtcLocal has a valid date
                    placeholders["pxDeliveryDate"] = hasValidDate && expectedDateOnUtcLocal != DateTime.MinValue
                        ? expectedDateOnUtcLocal.ToString("MM/dd/yyyy HH:mm:ss") // Format with hours, minutes, and seconds
                        : string.Empty; // Use an empty string or a default value
                    ...
                    placeholders["pxcusAdd"] = GetValueOrDefault("avpx_brl_streetaddress");
                    placeholders["pxcustomerCit"] = GetValueOrDefault("avpx_brl_city");
                    placeholders["pxcusS"] = GetValueOrDefault("avpx_brl_stateorprovince");
                    placeholders["pxcusP"] = GetValueOrDefault("avpx_brl_postalcode");
                    placeholders["pxcusCo"] = GetValueOrDefault("avpx_brl_country");
                ...
                placeholders["pxAccountName"] = accountName;
                placeholders["pxNotes"] = GetValueOrDefault("avpx_purpose");
                placeholders["pxSiteContact"] = onSiteContact;
                placeholders["pxEmail"] = emailContact;
                placeholders["pxPhone"] = contactTel;
            ...
            <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        {
                            Entity assetEntity = service.Retrieve("avpx_asset", assetId, new ColumnSet("avpx_make", "avpx_model", "avpx_serialnumbervin", "avpx_name", "av_unit", "avpx_recordid"));
                            ...
                            unitNumber = GetValueOrDefault(assetEntity, "avpx_recordid", "N/A");
                           
                        }
                        description = GetValueOrDefault(entity, "av_description", "N/A");
                        string[] recordData = new string[] { assetName, description, unitNumber, avpxSerialNumber };
            ...
            <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        description = GetValueOrDefault(entity, "av_description", "N/A");
                        string[] recordData = new string[] { description, avpxQuantity };
