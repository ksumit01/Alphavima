...
namespace AV.Prexa.Quote.PopulateWordTemplate
{
    public class PopulateWordTemplate : CodeActivity
    {
        ...
        static string miscellaneous = string.Empty;
        static string tearout = string.Empty;
        static string installation = string.Empty;
        static string serviceCalls = string.Empty;
        static string transportation = string.Empty;
        static string otherChargeName = string.Empty;
            ...
            var placeholders = new Dictionary<string, string>
    {
        ...
        {"pxSEmail",""}
            ...
            <link-entity name='account' from='accountid' to='avpx_account' link-type='outer' alias='ac'>
                ...
                string ownerTel = "";
                string ownerEmail = "";
                if (quote.Contains("avpx_customerscontact"))
                {
                    var contactReference = (EntityReference)quote["avpx_customerscontact"];
                ...
                if (quote.Contains("avpx_opportunity"))
                {
                    var oppReference = (EntityReference)quote["avpx_opportunity"];
                ...
                if (quote.Contains("ownerid"))
                {
                    var ownerReference = (EntityReference)quote["ownerid"];
                ...
                {
                    tracingService.Trace("No owner associated with this quote.");
                }
                // Assuming `quote` is of type Entity and has the method to get attributes.
                ...
                /*string ExtractTextFromHtml(string htmlContent)
                {
                    ...
                    // Select the text nodes and join them together
                    var text = doc.DocumentNode.InnerText;

                    // Optionally, trim the text and replace multiple spaces with a single space
                ...
                string GetAliasedValueOrDefault(string attribute, string defaultValue = "")
                {
                    try
                    {
                        if (quote.Contains(attribute) && quote[attribute] is AliasedValue aliasedValue)
                        {
               ...
               // placeholders["pxRentalSubtotal"] = GetFormattedValueOrDefault("avpx_detailedamount");
                ...
                placeholders["pxMRD"] = GetFormattedValueOrDefault("avpx_rentalperiod");
                placeholders["pxTMC"] = GetFormattedValueOrDefault("avpx_totalrentalamountnew");
                placeholders["pxCusName"] = GetAliasedValueOrDefault("ac.name");
                placeholders["pxConName"] = fullName;
                placeholders["pxJT"] = jobTitle;
                placeholders["pxN"] = fullName;
                placeholders["pxCon"] = contactTel;
        ...
        {
            tracing.Trace("Starting Post Byte64");
            string fileName = GenerateFileName(tracing);
            ...
            tracing.Trace("After File Name");
            using (var client = new HttpClient())
            {
                tracing.Trace("inside new HttpClient()");
        ...
        private string GenerateFileName(ITracingService tracingService)
        {
            string timestamp = DateTime.UtcNow.ToString("ddMMyyyy");
            Random random = new Random();
            string randomNumber = random.Next(1000, 10000).ToString();
            tracingService.Trace("Folder name generated successfully");
        ...
        static byte[] WordTemplate(byte[] fileData, byte[] imageData, Dictionary<string, string> placeholders, List<string[]> combinedData, ITracingService tracingService)
        {
            ...
            try
            {
                using (MemoryStream memoryStream = new MemoryStream())
                {
                 ...
                 ;*/
                        tracingService.Trace("miscellaneous" + miscellaneous);
                        tracingService.Trace("tearout" + tearout);
                        tracingService.Trace("installation" + installation);
                        tracingService.Trace("serviceCalls" + serviceCalls);
                        tracingService.Trace("transportation" + transportation);
                        placeholders["pxMCharge"] = miscellaneous;
                        placeholders["pxTCharge"] = tearout;
                        placeholders["pxICharge"] = installation;
                        placeholders["pxScCharge"] = serviceCalls;
                        placeholders["pxTrcharge"] = transportation;
        ...
        public List<string[]> RetrieveAndReturnRecords(IOrganizationService service, ITracingService tracingService, string recordId)
        {
            ...
            try
            {
                ...
                // Initialize strings for specific charge names
                string currencySymbol = "";
                decimal localmiscellaneous = 0;
                decimal localtearout = 0;
                decimal localinstallation = 0;
                decimal localserviceCalls = 0;
                decimal localtransportation = 0;
                string localotherChargeName = "0.00";
                ...
                if (entityCollection.Entities.Count > 0)
                {
                    ...
                    foreach (Entity entity in entityCollection.Entities)
                    {
                        ...
                        string avpxAmount = GetFormattedValueOrDefault(entity, "avpx_priceperunit", tracingService,ref currencySymbol);
                        string avpxExtendedAmount = GetFormattedValueOrDefault(entity, "avpx_amount", tracingService,ref currencySymbol);
                        ...
                        {
                            recordData = new string[] { description, avpxQuantity, avpxAmount, avpxExtendedAmount };
                            assetGroup.Add(recordData);
                        }
                        else if (avpxType == "Charge") // Charge
                        {
                            // Retrieve the asset group GUID
                            if (entity.Contains("avpx_assetgroup"))
                            {
                                EntityReference assetGroupRef = entity["avpx_assetgroup"] as EntityReference;
                                if (assetGroupRef != null)
                                {
                                    // Retrieve the asset group entity to get the charge name
                                    Entity assetGroupEntity = service.Retrieve("avpx_device", assetGroupRef.Id, new ColumnSet("avpx_chargename"));
                                    if (assetGroupEntity.Contains("avpx_chargename"))
                                    {
                                        string chargeName = assetGroupEntity["avpx_chargename"].ToString();
                                        tracingService.Trace("Charge Name" + chargeName);
                                        // Assign the charge name to the corresponding string
                                        if (chargeName.ToLower() == "miscellaneous")
                                            localmiscellaneous += GetRawValue(entity, "avpx_amount");
                                        else if (chargeName.ToLower() == "tearout")
                                            localtearout += GetRawValue(entity, "avpx_amount");
                                        else if (chargeName.ToLower() == "installation")
                                            localinstallation += GetRawValue(entity, "avpx_amount");
                                        else if (chargeName.ToLower() == "service calls")
                                            localserviceCalls += GetRawValue(entity, "avpx_amount");
                                        else if (chargeName.ToLower() == "transportation")
                                            localtransportation += GetRawValue(entity, "avpx_amount");
                                        if (chargeName.ToLower() == "dispatch")
                                            localtransportation += GetRawValue(entity, "avpx_amount");
                                        else
                                            localotherChargeName = chargeName;
                                    }
                                }
                            }
                    ...
                    tracingService.Trace("localmiscellaneous" + localmiscellaneous);
                    tracingService.Trace("localtearout" + localtearout);
                    tracingService.Trace("localinstallation" + localinstallation);
                    tracingService.Trace("localserviceCalls" + localserviceCalls);
                    tracingService.Trace("localtransportation" + localtransportation);
                    miscellaneous = FormatWithCurrency(localmiscellaneous, currencySymbol);
                    tearout = FormatWithCurrency(localtearout, currencySymbol);
                    installation = FormatWithCurrency(localinstallation, currencySymbol);
                    serviceCalls = FormatWithCurrency(localserviceCalls, currencySymbol);
                    transportation = FormatWithCurrency(localtransportation,currencySymbol);
                     otherChargeName = localotherChargeName;

                    tracingService.Trace("localtransportation" + transportation);
                }
                else
                {
