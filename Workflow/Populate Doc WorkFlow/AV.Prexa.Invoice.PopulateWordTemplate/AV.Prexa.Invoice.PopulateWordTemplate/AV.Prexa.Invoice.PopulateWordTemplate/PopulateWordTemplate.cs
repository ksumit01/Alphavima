    ...
    public class PopulateWordTemplate : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext executionContext)
        {
            ...
            try
            {
                ...
                // Call the method to retrieve Base64 PDF and Word data
                var result = RetrieveEntityColumnData(service, new Guid(docTemplateID), recordId, tracingService, context).Result;
                string base64Pdf = result.base64Pdf;
                string base64Word = result.base64Word;

                tracingService.Trace("Data retrieved successfully.");

                // Set the output arguments
                Base64Pdf.Set(executionContext, base64Pdf);
                Base64Word.Set(executionContext, base64Word);
        ...
        private async Task<(string base64Pdf, string base64Word)> RetrieveEntityColumnData(
            IOrganizationService service, Guid docTemplateID, string recordID,
            ITracingService tracingService, IWorkflowContext context)
        {
            ...
            try
            {
                ...
                if (fileData != null)
                {
                    ...
                    string base64Pdf = await PostBase64ToPowerAutomate(updatedWordDataString, powerAutomateUrl);
