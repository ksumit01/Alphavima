    ...
    public class PopulateWordTemplate : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext executionContext)
        {
            ...
            try
            {
                ...
                // Retrieve both Base64 Pdf and Word
        ...
        private async Task<(string base64Pdf, string base64Word)> RetrieveEntityColumnData(
            IOrganizationService service, Guid docTemplateID, string recordID, ITracingService tracingService, IWorkflowContext context)
        {
            ...
            try
            {
                ...
                if (fileData != null)
                {
                    ...
                    string base64Word = Convert.ToBase64String(updatedWordData);

                    // Post processed data to Power Automate to get the PDF
                    tracingService.Trace($"Posting processed Word data to Power Automate URL: {powerAutomateUrl}");
                    string base64Pdf = await PostBase64ToPowerAutomate(base64Word, powerAutomateUrl);

                    return (base64Pdf, base64Word);
                }
                else
                {
